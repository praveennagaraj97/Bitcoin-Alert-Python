...
import formatdata
# time module is used to extract sleep method to set the time between data collection.
from time import sleep
# requests is used to post and create sessions
import requests
# this function takes event name as parameter and sent the data to IFTTT applet.
def sendtoIFTTT(event='Bitcoin_price'):
  # empty list is created to store the data from format data module
  datahistory = []
  # i is the variable to show in console that - how many times the data is collected
  i = 1
  ...
  while True:
    ...
    ifttt_url = f'https://maker.ifttt.com/trigger/{event}/with/key/jeoII-tpwkkU0O5W4-xDTGYzk-V0mx7PCOMhblx-pGR'

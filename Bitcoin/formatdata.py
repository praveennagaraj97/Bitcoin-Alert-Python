...
# since IFTTT has difeerent time zone pytz module is used to get the
# required timezone.
from pytz import timezone
# country is asked once at the time of starting server
country = input('Enter Country Code : ')

# this function will format the data(from get_price_from_url module) and
# return the formated data into function.


def formatdata():
    try:
        # data is taken from get_price_from_url module and stored in data
        # variable.
        data = get_price_from_url.get_data(country)
        # this is the format of date and time which will be recieved in
        # telegram
